...
function priority(r){return `<span class="priority ${cls(r.priority)}">${r.priority==='Top'?'â˜… ':r.priority==='High'?'â†‘ ':''}${esc(r.priority)}</span>`;}
function role(r){return `<button class="role-button" data-edit="${esc(r.id)}">${esc(r.role)}</button>`;}
function link(r){return safeLink(r.link)?`<a class="job-link" href="${esc(safeLink(r.link))}" target="_blank" rel="noopener noreferrer" aria-label="Open posting for ${esc(r.company+' '+r.role)}">Posting â†—</a>`:'<span class="subtext">No link</span>';}
...
render();if(loadError){$('#save-state').textContent='Recovery needed Â· export backup';$('#save-state').classList.add('error-state');toast(loadError);}else persist();

function saveSnapshot(){
 if(hosted)return;
 const body=JSON.stringify({schemaVersion:1,applications:rows,syncState});
 snapshotQueue=snapshotQueue.catch(()=>{}).then(async()=>{const response=await fetch('/api/snapshot',{method:'POST',headers:{'Content-Type':'application/json'},body});if(!response.ok)throw Error('Snapshot failed');}).catch(()=>{$('#sync-status').textContent='Browser saved Â· job-watch copy unavailable';});
}
async function pullUpdates(){
 ...
 try{
  ...
  const review=syncState.review.map(r=>`${r.company} â€” ${r.role}: ${r.reason}`);
 ...
 }catch{$('#sync-status').textContent='Updates unavailable Â· your saved roles are safe';}
...
async function pullPublicUpdates(){
 ...
 try{
  ...
  $('#sync-details').innerHTML='<p>Chat publishes company names, roles and statuses after job-watch updates. This page checks every minute. Your notes, priorities, manual status overrides and deletions stay in this browser. Browser edits are not sent to GitHub. Ask chat to update the shared checklist.</p>';
